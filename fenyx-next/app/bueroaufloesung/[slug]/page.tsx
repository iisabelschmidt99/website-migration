// Öffentliche Route: /bueroaufloesung/<slug> (Themen-Collection).
...
const COLLECTION = "bueroaufloesung";
...
export default async function BueroaufloesungTopicPage({ params }: PageProps) {
  const { slug } = await params;
  const page = await getTopicPage(COLLECTION, slug);
  if (!page) notFound();
