// Öffentliche Route: /bueroeinrichtung/<slug> (Themen-Collection).
...
const COLLECTION = "bueroeinrichtung";
...
export default async function BueroeinrichtungTopicPage({ params }: PageProps) {
  const { slug } = await params;
  const page = await getTopicPage(COLLECTION, slug);
  if (!page) notFound();
