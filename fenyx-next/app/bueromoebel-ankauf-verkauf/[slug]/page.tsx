// Öffentliche Route: /bueromoebel-ankauf-verkauf/<slug> (kanonisch, Zielbild SEO).
// Inhalt aus Supabase (landing_locations, collection = 'ankauf').
...
export default async function BueromoebelAnkaufVerkaufStadtPage({ params }: PageProps) {
  const { slug } = await params;
  const page = await getPage(slug);
  if (!page) notFound();
