...
// Variante B — verzweigter Survey-Wizard (nur Formularspalte, ohne Portrait).
...
export type SurveyContactFormProps = {
  email: string;
  phone: string;
  headingId?: string;
};

export default function SurveyContactForm({
  email,
  phone,
  headingId = "survey-kontakt-heading",
}: SurveyContactFormProps) {
  ...
  return (
    <>
      ...
      <h2 id={headingId} className="survey__headline">
        Was dürfen wir <span className="survey__hl">für Sie</span> tun?
      </h2>
      <p className="survey__subline">
        Ein paar Klicks genügen – wir melden uns mit einem{" "}
        <span className="survey__hl">passenden Vorschlag</span> für Ihr Büro.
      </p>
      ...
      {submitted ? (
        ...
        <div className="survey__panel" key="contact">
          {step.privat ? (
            ...
            >
              ...
              Wir bedienen ausschließlich Geschäftskunden. Für private Anliegen
              arbeiten wir mit einem ausgewählten Partner zusammen – wir leiten
              Ihre Anfrage gerne dorthin weiter.
            </div>
          ) : (
            <div
              className="mb-5 border-l-2 border-signal bg-signal/10 px-4 py-3 text-sm leading-relaxed text-white/85"
              role="note"
            >
              <strong className="text-signal font-semibold">Gut zu wissen:</strong>{" "}
        ...
        <div className="survey__panel" key="size">
          ...
          <p className="survey__hint">
            Geben Sie die Fläche oder die Zahl der Arbeitsplätze an – eine grobe
            Schätzung genügt.
          </p>
          ...
          <div className="survey__choices">
            {step.options.map((option) => {
              ...
              return (
                ...
                >
                  <span className="survey__choice-radio" aria-hidden="true" />
                  <span className="survey__choice-label">{option.label}</span>
                </button>
              );
            })}
          </div>
      ...
      {!submitted && (
        <div className="survey__nav">
          {clampedIndex > 0 ? (
            <button type="button" className="survey__back" onClick={back}>
              ← Zurück
            </button>
          ) : (
            <span />
          ...
          {step.kind === "contact" ? (
            ...
            <button type="button" className="survey__next" onClick={goNext}>
              Weiter →
            </button>
          ) : (
