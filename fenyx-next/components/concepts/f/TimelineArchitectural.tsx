...
// Konzept F — Timeline als 3 numbered Kapitel, alternating abyss/mist-soft.
...
export default function TimelineArchitectural() {
  return (
    <section
      className="df-timeline df-root"
      aria-label="Leistungen in drei Kapiteln"
    >
      {CHAPTERS.map((chapter) => (
        <article
          key={chapter.index}
          className={`df-tl-chapter df-tl-chapter--${chapter.tone}`}
          aria-labelledby={`df-tl-${chapter.index}-heading`}
        >
          <div className="df-tl-chapter__media df-img">
            <Image
          ...
          <div className="df-tl-chapter__body">
            <p className="df-tl-chapter__index" aria-hidden="true">
              {chapter.index}
            </p>
            <p className="df-tl-chapter__eyebrow">{chapter.eyebrow}</p>
            <WordReveal
              tag="h2"
              text={chapter.title}
              className="df-tl-chapter__title"
              id={`df-tl-${chapter.index}-heading`}
            />
            <Reveal tag="p" className="df-tl-chapter__body-text" delay={400}>
              {chapter.body}
            </Reveal>
            <SpecReveal>
              {chapter.specs.map((spec, i) => (
                <div key={spec.label} className="df-spec__row">
                  <dt className="df-spec__label">{spec.label}</dt>
                  <dd
                    className="df-spec__value"
                    style={{ "--df-i": i } as React.CSSProperties}
                  >
                    {spec.value}
                  </dd>
                </div>
