...
// Konzept I (Klon von F) — Referenzen als technisches Bento-Raster.
...
export default function ReferencesArchitectural({ projects }: Props) {
  ...
  return (
    <section className="di-refs di-root" aria-labelledby="di-refs-heading">
      <div className="di-refs__intro">
        <Rise tag="p" className="di-refs__eyebrow">
          <span>03 / 03 — Ergebnisse aus der Praxis</span>
        </Rise>
        <Rise tag="h2" className="di-refs__heading" delay={120}>
          <span id="di-refs-heading">Was bleibt, wenn ein Büro neu gedacht wird.</span>
        </Rise>
      </div>

      <Stagger className="di-refs__grid" step={110} y={36}>
        {tiles.map((project, index) => {
          ...
          return (
            <Link
              ...
              className={`di-ref-tile anim-stagger ${span ? `di-ref-tile--${span}` : ""}`}
            ...
            >
              ...
              >
                <Image
                  ...
                  fill
                  className="object-cover object-center di-ref-tile__img"
                  sizes={span === "big" ? "(max-width: 768px) 100vw, 50vw" : "(max-width: 768px) 100vw, 25vw"}
                  loading="lazy"
                />
              </ClipReveal>
              <div className="di-ref-tile__scrim" aria-hidden="true" />
              <div className="di-ref-tile__body">
                <p className="di-ref-tile__eyebrow">{project.eyebrow}</p>
                <h3 className="di-ref-tile__company">{project.heading}</h3>
                {kpi && (() => {
                  const { number, caption } = splitKpi(kpi);
                  return (
                    <div className="di-ref-tile__kpi">
                      <CountUp className="di-ref-tile__kpi-value" value={number} />
                      <span className="di-ref-tile__kpi-label">{caption}</span>
                    </div>
                  );
                })()}
              </div>
